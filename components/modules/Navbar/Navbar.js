...
function Navbar() {
  ...
  useEffect(() => {
    // اگر query.q وجود داشته باشد، مقدار آن را در کادر جستجو قرار می‌دهد
    setSearch(route.query.q || "");
  }, [route.query.q]); // وابسته به تغییرات query.q
  ...
  const searchHandler = () => {
    if (search.trim()) {
      ...
      setSearch(''); // پاک کردن کادر جستجو بعد از جستجو
    }
  };
  ...
  return (
    <div className={`container-fluid p-0 ${styles.nav_bar}`}>
      ...
      >
        <div className={styles["d-flex align-items-center position-relative"]}>
          <Link href="/" className={`${styles.navbar_brand} px-lg-4 m-0`}>
            <h1 className={`${styles["text-white"]} m-0 display-4 text-uppercas `} >
              Next-Coffee
            </h1>
          </Link>
